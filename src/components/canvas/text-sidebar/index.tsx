...
const TextSideBar = ({ isOpen }: Props) => {
    ...
    // Early return if Redux state isn't initialized
    ...
    const updateTextProperty = (property: keyof TextShape, value: any) => {
        ...
        // Dispatch an action to update the text property of the selected text shape
        dispatch(
            updateShape({
    ...
    const [colorInput, setColorInput] = useState(selectedTextShape?.fill || '#000000')
    ...
    return (
        ...
        <div className={cn('fixed right-5 top-1/2 transform -translate-y-1/2 w-80 backdrop-blur-xl  bg-white/[0.08] border-white[0.12]gap-2 p-3 saturate-150 border rounded-lg z-50 transition-transform duration-300',
            ...
            <div className='p-4 flex flex-col gap-10 overflow-y-auto max-h-[calc(100vh-8rem)]'>
                    ...
                    <Select value={selectedTextShape.fontFamily}
                        ...
                        <SelectContent className='bg-black/90 border-white/10'>
                            {fontFamilies.map((font) => (
                                ...
                                >
                                    <span style={{ fontFamily: font }}>{font.split(',')[0]}
                ...
                <div className='space-y-3'>
                    ...
                    <div className='flex gap-2'>
                        <Toggle pressed={selectedTextShape.fontWeight >= 600}
                            onPressedChange={(pressed) => updateTextProperty('fontWeight', pressed ? 700 : 400)}
                            className='data-[state=on]:bg-blue-500 data-[state=on]:text-white'
                        >
                            <Bold className='w-4 h-4' />
                        </Toggle>
                        <Toggle pressed={selectedTextShape.fontStyle === 'italic'}
                        ...
                        <Toggle pressed={selectedTextShape?.textDecoration === 'underline'}
                            onPressedChange={(pressed) => updateTextProperty('textDecoration', pressed ? 'underline' : 'none')}
                            className='data-[state=on]:bg-blue-500 data-[state=on]:text-white'
                        >
                            <Underline className='w-4 h-4' />
                        </Toggle>
                        <Toggle pressed={selectedTextShape.textDecoration === 'line-through'}
                ...
                <div className='space-y-2'>
                    ...
                    <div className='flex gap-2'>
                        <Input value={colorInput} onChange={(e) => handleColorChange(e.target.value)}
                        placeholder='#FFFFFF'
