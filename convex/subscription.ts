...
export const upsertFromPolar = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        const existingByPolar = await ctx.db.query('subscriptions')
            .withIndex('by_polarSubscriptionId', (q) => q.eq('polarSubscriptionId', args.polarSubscriptionId))
            .first()
        const existingByUser = await ctx.db.query('subscriptions')
        ...
        const base = {
            ...
            creditsGrantPerPeriod: args.creditsGrantPerPeriod ?? existingByPolar?.creditsGrantPerPeriod ??
                existingByUser?.creditsGrantPerPeriod ?? DEFAULT_GRANT,
            creditsRolloverLimit: args.creditsRolloverLimit ?? existingByPolar?.creditsRolloverLimit ??
                existingByUser?.creditsRolloverLimit ?? DEFAULT_ROLLOVER_LIMIT,
        }
        if (existingByPolar) {
            if (existingByPolar.userId === args.userId) {
                ...
                const userExistingSubscription = await ctx.db.query('subscriptions')
                ...
                if (userExistingSubscription) {
                    ...
                    console.log('[Convex] Created new subscription with ID:', newId)
                }
            }
        }
        if (existingByUser) {
...
export const grantCreditsIfNeeded = mutation({
    ...
    handler: async (ctx, {
        ...
        const dupe = await ctx.db.query('credits_ledger')
            .withIndex('by_idempotencyKey', (q) => q.eq('idempotencyKey', idempotencyKey))
            .first()

        if (dupe) return { ok: true, skipped: true, reason: 'duplicate-ledger' }
        const sub = await ctx.db.get(subscriptionId)
        if (!sub) return { ok: false, skipped: true, reason: 'no-subscription' }

        if (sub.lastGrantCursor === idempotencyKey) {
            return { ok: true, skipped: true, reason: 'cursor-matched' }
        }

        if (!ENTITLED.has(sub.status)) {
            return { ok: false, skipped: true, reason: 'not-entitled' }
        }
...
export const consumeCredits = mutation({
    ...
    handler: async (ctx, { userId, amount, reason, idempotencyKey }) => {
        ...
        if (idempotencyKey) {
            const dupe = await ctx.db
                ...
                .withIndex('by_idempotencyKey', (q) =>
                  q.eq('idempotencyKey', idempotencyKey)).first()
        ...
        if (sub.creditsBalance < amount) return { ok: false, error: 'insufficient-credits', balance: sub.creditsBalance }
